...
public class TextDispose {
    ...
    private static final char[] CHINESE_UNIT = {'里', '分', '角', '元', '拾', '佰', '仟', '万', '拾', '佰', '仟', '亿', '拾', '佰', '仟', '万', '拾', '佰', '仟', '兆', '拾', '佰', '仟', '万', '拾', '佰', '仟'};

    /**
     * 返回关于钱的中文式大写数字,支仅持到兆
     */
    public static String arabNumberToChinese(long moneyNum) {
        ...
        while (moneyNum > 0) {
            ...
            res.insert(0, CHINESE_NUM[Math.toIntExact(moneyNum % 10)]);
            moneyNum /= 10;
        }
        return (res.toString()
                .replaceAll("零[拾佰仟]", "零")
     ...
     * 返回关于钱的中文式大写数字,支仅持到兆
     */
    public static String arabNumberToChinese(int moneyNum) {
        StringBuilder res = new StringBuilder();
        int i = 3;
        if (moneyNum == 0) {
            return "零元";
        ...
        while (moneyNum > 0) {
            res.insert(0, CHINESE_UNIT[i++]);
            res.insert(0, CHINESE_NUM[moneyNum % 10]);
            moneyNum /= 10;
        }
        return (res.toString()
                .replaceAll("零[拾佰仟]", "零")
     ...
     */
    public static String arabNumberToChinese(double moneyNum) {
        StringBuilder res = new StringBuilder();
        String money = String.format("%.3f", moneyNum);
        int i = 0;
        String zeroChinese = "零";
        BigDecimal moneyNumBigDecimal = new BigDecimal(moneyNum);
        BigDecimal zeroBigDecimal = new BigDecimal("0.0");
        if (zeroBigDecimal.equals(moneyNumBigDecimal)) {
            return "零元";
        }
        ...
        while (deci > 0) {
            res.insert(0, CHINESE_UNIT[i++]);
            res.insert(0, CHINESE_NUM[deci % 10]);
            deci /= 10;
        }
        res = new StringBuilder(res.toString().replaceAll("零[里分角]", "零"));
        int rmbDecimal = 3;
        if (i < rmbDecimal) {
            res.insert(0, "零");
        }
        res = new StringBuilder(res.toString().replaceAll("零+", "零"));
        if (res.toString().endsWith(zeroChinese)) {
            res = new StringBuilder(res.substring(0, res.length() - 1));
        }
        try {
            return arabNumberToChinese(inte) + ("".equals(res.toString()) ? "整" : (res.toString().contains("角") && !res.toString().contains("分") && !res.toString().contains("里")) ? res + "整" : res.toString());
        } catch (Exception e) {
            return e.getMessage();
        }
    }
     ...
     * 返回关于钱的中文式大写数字,支仅持到兆
     */
    private static String arabNumberToChinese(String moneyNum) throws Exception {
        StringBuilder res = new StringBuilder();
        int i = 3;
        int maxDigit = 17;
        String zero = "0";
